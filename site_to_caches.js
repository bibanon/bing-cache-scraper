...
async.doUntil((cb) => {
    ...
    utils.wait(utils.DELAY)
        ...
        .then((window) => {
            let info = Array.prototype.slice.call(window.document.querySelectorAll('.b_algo .b_attribution'));
            info.forEach((result) => {
                let cacheAttr = result.getAttribute('u');
                if (cacheAttr !== null) {
                    let cacheParams = result.getAttribute('u').split('|');;
                    currentResults.push('http://cc.bingj.com/cache.aspx?d=' + cacheParams[2] + '&mkt=en-US&setlang=en-US&w=' + cacheParams[3]);
                }
            });
